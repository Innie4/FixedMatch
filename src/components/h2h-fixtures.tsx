...
export default function H2HFixtures({ homeTeam, awayTeam }: H2HFixturesProps) {
  ...
  const homeWins = h2hFixtures.filter(
    (f) =>
      (f.homeTeam === homeTeam && f.homeScore > f.awayScore) ||
      (f.awayTeam === homeTeam && f.awayScore > f.homeScore)
  ).length

  const awayWins = h2hFixtures.filter(
    (f) =>
      (f.homeTeam === awayTeam && f.homeScore > f.awayScore) ||
      (f.awayTeam === awayTeam && f.awayScore > f.homeScore)
  ).length
        ...
        <div className="grid grid-cols-3 w-full max-w-md mx-auto text-center">
          ...
          <div>
            ...
            <div className="text-sm text-gray-600 dark:text-gray-400">
              Here&apos;s the head-to-head record between these teams
            </div>
          </div>
        </div>
      </div>
