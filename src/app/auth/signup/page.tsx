...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Reset error
    // setError(null) // Removed, now using toasts
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 && data.errors) {
          Object.values(data.errors).forEach((errorMsgs: any) => {
            toast({
      ...
      if (result?.error) {
        ...
        toast({
          title: 'Success',
          description: 'Account created and logged in successfully! Redirecting...',
          variant: 'default',
        })
        router.push('/')
      }
      ...
      toast({
        title: 'Error',
        description: err.message || 'An unexpected error occurred during signup. Please try again.',
        variant: 'destructive',
      })
    } finally {
      setIsLoading(false)
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-400 mt-1">
              Sign up to get started with Legit Soccer Tips
            </p>
          </div>

          {/* Error display removed, now using toasts */}
