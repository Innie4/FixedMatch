...
export default function PaymentMethodsPage() {
  ...
  const getCountryPaymentMethods = (): CountryPaymentMethods[] => {
    return countries.map(country => {
      const countryMethods = paymentMethods.filter(method => 
        method.countries.includes(country.code) && method.status === "active"
      )
      return {
        countryCode: country.code,
        countryName: country.name,
        paymentMethods: countryMethods,
        defaultCurrency: countryMethods.length > 0 ? countryMethods[0].supportedCurrencies[0] : "USD",
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(paymentMethods.reduce((acc, m) => acc + m.transactionFee, 0) / paymentMethods.length).toFixed(1)}%
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="methods" className="space-y-4">
              ...
              <TableBody>
                {paginatedMethods.map((method) => (
                  <TableRow key={method.id}>
                    ...
                    <TableCell>
                      <div className={`flex items-center gap-1 ${getTestStatusColor(method.testStatus)}`}>
                        <div className={`w-2 h-2 rounded-full ${
                          method.testStatus === "success" ? "bg-green-600" :
                          method.testStatus === "failed" ? "bg-red-600" :
                          method.testStatus === "pending" ? "bg-yellow-600" : "bg-gray-400"
                        }`} />
          ...
          <div className="flex items-center justify-between">
            <div className="text-sm text-muted-foreground">
              Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredMethods.length)} of {filteredMethods.length} results
            </div>
            <div className="flex items-center space-x-2">
              <Button
                variant="outline"
        ...
        <TabsContent value="countries" className="space-y-4">
          <div className="grid gap-4">
            {getCountryPaymentMethods().slice(0, 20).map((countryData) => (
              <Card key={countryData.countryCode}>
                ...
                <CardContent>
                  {countryData.paymentMethods.length > 0 ? (
                    ...
                    <div className="text-sm text-muted-foreground">
                      No payment methods assigned. Users will see USD pricing with general payment options.
                    </div>
                  )}
                </CardContent>
              </Card>
      ...
      <TestConnectionDialog
        ...
        onTestComplete={(methodId, status) => {
          setPaymentMethods(methods => methods.map(method => 
            method.id === methodId 
              ? { ...method, testStatus: status, lastTested: new Date().toISOString().split('T')[0] }
              : method
          ))
        }}
      />
